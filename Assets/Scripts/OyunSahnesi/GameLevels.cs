...
public struct Level
{
    ...
    public Level(List<string> kalip, bool kilitliKutu, bool hareketsizKutu, bool odulKutus, int renkSayisi,
        int meyveSayisi, int hamleLimiti, int levelGecmePuani)
    {
        Kalip = kalip;
...
public static class GameLevels
{
    ...
    public static void SetLevel()
    {
        ...
        if (IsaretleBelirtYoket.Instance.HamleSayisi >= GetLevel().HamleLimiti)
        {
            AsilanLimitAdi = "HamleLimiti"; 
            GameManager.Instance.PopUplar.Goster(AsilanLimitAdi); 
        ...
        {
            Debug.Log("OynananMeyveNo Limiti Aştı");
            OynananMeyveNo = 3;
            PlayerPrefs.SetInt("OynananMeyveNo", OynananMeyveNo);
            OynananRenkNo++;
            ...
            {
                Debug.Log("OynananRenkNo Limiti Aştı");
                ...
                {
                    Debug.Log("OynananKalipNo Limiti Aştı");
                    OynananKalipNo = 0;
                    PlayerPrefs.SetInt("OynananKalipNo", OynananKalipNo);
                    GamePlayLevelID++;
                    ...
                    if (GamePlayLevelID > RecordLevelID)
                    {
                        RecordLevelID++;
                        PlayerPrefs.SetInt("RecordLevelID", RecordLevelID);
                        Debug.Log($"GamePlayLevelID Limiti Aştı RecordLevelID : {RecordLevelID}");
                    }
                }
            }
        }
